...
serviceRouter.post("/person", function(request, response) {
    ...
    const personDao = new PersonDao(request.app.locals.dbConnection);
    try {
        var kunde = personDao.loadByName(request.body.Vorname, request.body.Nachname, request.body.Plz);
        if(kunde == null){
            var result = personDao.create(request.body.Vorname, request.body.Nachname, request.body.Ort, request.body.Plz, request.body.Strasse, request.body.Hausnummer);
            helper.log("Service Person: Record inserted");
            response.status(200).json(helper.jsonMsgOK(result));
        } else {
            helper.log("Service Person: User loaded");
            response.status(200).json(helper.jsonMsgOK(kunde));
        }
    } catch (ex) {
