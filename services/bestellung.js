...
serviceRouter.get("/bestellung/letzteid/", function(request, response){
    helper.log("Service Bestellung: Client requested last id");

    const bestellungDao = new BestellungDao(request.app.locals.dbConnection);
    try{
        var result = bestellungDao.getLastID();
        helper.log("Service Bestellung: Records loaded, id=" + result.ID);
        response.status(200).json(helper.jsonMsgOK(result));
    }
    catch(ex){
        helper.logError("Service Bestellung: Error with last id. Exception occured: " + ex.message);
        response.status(400).json(helper.jsonMsgError(ex.message));
    }
});
...
serviceRouter.post("/bestellung", function(request, response) {
    ...
    var errorMsgs=[];
    // "KundenID", "ArtikelIDs", "Zahlungsmethode"
    ...
    if(helper.isUndefined(request.body.KundenID))
        errorMsgs.push("KundenID nicht gesetzt!");
    if(helper.isUndefined(request.body.ArtikelIDs))
        errorMsgs.push("Keine Artikel gefunden");
    if(helper.isUndefined(request.body.Zahlungsmethode))
        errorMsgs.push("Keine Zahlungsmethode gesetzt!");
    ...
    console.log(request.body.ArtikelIDs);

    const bestellungDao = new BestellungDao(request.app.locals.dbConnection);
    try {
        var result = bestellungDao.create(request.body.Bestellzeitpunkt, request.body.KundenID, request.body.ArtikelIDs, request.body.Zahlungsmethode);
