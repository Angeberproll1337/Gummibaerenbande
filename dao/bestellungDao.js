...
class BestellungDao {
    ...
    // request.body.Bestellzeitpunkt, request.body.KundenID, request.body.ArtikelIDs, request.body.Zahlungsmethode
    create(Bestellzeitpunkt = null, KundenID = null, ArtikelIDs = [], Zahlungsmethode = null) {
        const bestellpositionDao = new BestellpositionDao(this._conn);
        ...
        var sql = "INSERT INTO Bestellung (Bestellzeitpunkt,KundenID,ZahlungsartID) VALUES (?,?,?)";
        var statement = this._conn.prepare(sql);
        var params = [helper.formatToSQLDateTime(Bestellzeitpunkt), KundenID, Zahlungsmethode];
        var result = statement.run(params);

        if (result.changes != 1) 
            throw new Error("Could not insert new Record. Data: " + params);

        for (var element of ArtikelIDs) {
            bestellpositionDao.create(result.lastInsertRowid, element[1], element[0]);
        }
    ...
    update(id, bestellzeitpunkt = null, bestellerid = null, zahlungsartid = null, bestellpositionen = []) {
        ...
        var sql = "UPDATE Bestellung SET Bestellzeitpunkt=?,KundenID=?,ZahlungsartID=? WHERE ID=?";
        var statement = this._conn.prepare(sql);
