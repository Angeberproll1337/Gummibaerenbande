...
class PersonDao {
    ...
    loadById(id) {
        ...
        var sql = "SELECT * FROM Kunde WHERE KundenID=?";
    ...
    loadByName(Vorname, Nachname, Plz) {
        var sql = "SELECT * FROM Kunde WHERE Vorname=? AND Nachname=? AND PLZ=?";
        var statement = this._conn.prepare(sql);
        var params = [Vorname, Nachname, Plz];
    ...
    create(vorname = "", nachname = "", ort = "", Plz = 0, strasse="", hausnummer=0) {
        var sql = "INSERT INTO Kunde (Vorname,Nachname,Ort,PLZ,Strasse,Hausnummer) VALUES (?,?,?,?,?,?)";
        var statement = this._conn.prepare(sql);
        var params = [vorname, nachname, ort, Plz, strasse, hausnummer];
